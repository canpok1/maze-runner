...
while true; do
  # draftラベルなし・in-progress-by-claudeラベルなし・open状態のissueを全件取得
  if issues=$(gh issue list --search "-label:draft -label:$IN_PROGRESS_LABEL" --state open --limit 1000 --json number --jq '.[].number'); then
    if [ -n "$issues" ]; then
      # 各issueについてサブIssueの有無を確認
      for issue_number in $issues; do
        ...
        if sub_issue_count=$(gh api "/repos/${REPO_OWNER}/${REPO_NAME}/issues/${issue_number}/sub_issues" --jq 'length' 2>/dev/null); then
          ...
          if [ "$sub_issue_count" = "0" ]; then
            ...
            # 処理開始：in-progress-by-claudeラベルを付与
            if gh issue edit "$issue_number" --add-label "$IN_PROGRESS_LABEL"; then
              ...
              if claude --remote "/breakdown-story ${issue_number}"; then
                ...
                echo "エラー: issue #${issue_number} のストーリー分解に失敗しました" >&2
              fi
