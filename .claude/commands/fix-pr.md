...
3. CIの終了を待機する。
    - `gh pr checks <PR番号> --watch` でPRの全チェックが完了するまで待機する。
...
9. 完了条件を全て満たしたら、PRのマージを行う。
    - `github` スキル（`pr-merge.sh`）を使用してPRがマージ可能か確認する。
    - マージ可能な場合、`pr-merge.sh` をマージ方式を指定せずに実行する（リポジトリのデフォルト設定に従う）。
    - マージ完了後、マージが完了した旨をユーザーに報告する。
    - マージ不可の場合、マージできない旨とその理由をユーザーに報告し、作業を中断する。
