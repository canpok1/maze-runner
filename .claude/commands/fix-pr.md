...
4. `github` スキルを使用してCIの結果を詳細に把握する。
    ...
    - 一時的な問題（インフラ障害、flaky test等）と判断した場合は、`gh run rerun <run-id>` で失敗したワークフローを再実行し、手順3に戻ってCIの完了を再度待機する。ただし、同一ワークフローの再実行は最大2回までとする。
...
7. 手順4と5で決めた方針に従って対応を行う。
    ...
    - 各レビューコメントスレッドへ返信を行う。
8. 完了条件を満たしているか確認する。
    - CIが未完了の場合は、`gh pr checks <PR番号> --watch` でCIの完了を待機してから手順2に戻る。
    - CI以外の完了条件（レビュースレッドの解決など）を満たしていない場合は、待機理由（未完了の条件）をユーザーに報告してから手順2に戻る。
    - ただし、手順2からの再実施は最大10回まで繰り返す。
