...
2. マージしてよいかユーザーに確認する。
    - AskUserQuestion を使用する。
    - 確認メッセージ例: 「以下のPRをマージしてもよろしいですか？」
3. ユーザーが承認した場合、`mcp__github__merge_pull_request` でマージする:
...
1. 「未着手・着手可能」に分類されたタスクの一覧を提示する:
    - Issue番号、タイトル、URL
2. `assign-to-claude` ラベルを付与してよいかユーザーに確認する。
    - AskUserQuestion を使用する。
    - 確認メッセージ例: 「以下のタスクに `assign-to-claude` ラベルを付与してもよろしいですか？」
3. ユーザーが承認した場合、以下を実行する:
