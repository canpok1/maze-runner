...
3. 残ったIssueについて「Issue内容の有効性チェック」を実施する（可能な限り並列実行）:
   ...
   - 以下の観点で有効性を検証する:
     ...
     - **マージ済みPRによる重複確認**: `github` スキルの `pr-search.sh` を使用して、Issue番号に関連するマージ済みPRを検索する（検索クエリ例: `is:merged "#<Issue番号>"`）。該当するPRが見つかった場合、PRの内容を確認し、同等の変更が既にマージされていないか検証する。マージ済みの場合は「対応不要の可能性あり」と判断する。
