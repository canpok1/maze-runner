...
5. `github` スキルを使用してレビューコメントを把握し、対応方針を決める。
    ...
    - botレビュアーからのコメントかつ対応済み（コード修正・返信完了）: Resolve
...
9. 完了条件を全て満たしたら、PRのマージを行う。
    ...
    - マージ結果をユーザーに報告する。失敗した場合は理由も報告し、作業を中断する。
...
- 承認されるまではResolveすることは禁止
  - ただし、以下の場合は例外としてResolve可能：
    - IsOutdated=true かつ改修提案への対応済み（コード修正・返信完了）のスレッド
    - botレビュアーからのコメントで、かつ改修提案への対応済み（コード修正・返信完了）のスレッド
