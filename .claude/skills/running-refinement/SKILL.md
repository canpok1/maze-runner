...
5. PMエージェントの判断結果を一覧形式でユーザーに提示し、確認を得る:
   ...
   - ユーザーが判断を覆したい場合は変更を反映する。
6. 対応必要と判断されたIssueについて以下を実行する:
   - `github` スキル（`issue-get.sh`）で現在のラベル一覧を取得
   - PMエージェントが判断した `story` または `task` ラベルを追加
   - `github` スキル（`issue-update.sh`）の `--add-label` オプションでラベルを更新
7. 対応不要と判断されたIssueについて以下を実行する:
