...
description: リポジトリ全体のバックログリファインメント（ラベル最適化、ストーリー細分化、フィードバック確認、タスクアサイン）を一括実行します。バックログを整理してClaude自動対応の準備を整える場合に使用してください。
layer: workflow
---
...
1. `github` スキルを使用して `story` ラベル付きのオープンIssue一覧を取得する（`gh issue list --repo "${OWNER}/${REPO}" --state open --label story --json number,title --limit 100`）。
2. 各ストーリーについて以下の情報を取得する（可能な限り並列実行）:
   - `github` スキル（`issue-comments.sh`）でコメント一覧を取得
   - `github` スキル（`issue-sub-issues.sh`）でサブIssue一覧を取得
3. 各ストーリーについて、サブIssueが存在しない場合はスキップする（細分化前のストーリーはフィードバック確認の対象外）。
4. 各ストーリーのコメントから未対応フィードバックを検出する。検出条件は以下の全てを満たすこと:
   - コメントの `author_association` が `OWNER`、`COLLABORATOR`、`MEMBER` のいずれか
   - コメントの `created_at` が、そのストーリーの最新サブIssue作成日時より後である（最新サブIssue作成日時は `github` スキル（`issue-get.sh`）で各サブIssueの詳細を取得して確認する）
   - コメント内容に改善要望・変更依頼・不備指摘が含まれる（`pm-specialist` エージェントで判定する）
5. 未対応フィードバックが検出されなかった場合は「未対応フィードバックなし」と報告し、フェーズ4に進む。
6. 検出された未対応フィードバックについて、各ストーリーごとに以下を実施する:
   - `pm-specialist` エージェントにフィードバック内容を提供し、必要な対応を判断させる:
     - **受け入れ条件の更新が必要な場合**: `github` スキル（`issue-update.sh`）でストーリーIssueの本文を更新する
     - **追加タスクが必要な場合**: `document-specialist` エージェントにタスクIssueの本文生成を依頼し、`github` スキル（`issue-create.sh`、`--label task` オプション）でIssueを作成、`github` スキル（`sub-issue-add.sh`）でストーリーのサブIssueとして登録する
7. 処理結果をストーリーごとに報告する（ストーリーIssue番号、検出フィードバック数、対応内容: 受け入れ条件更新/追加タスク作成/対応不要）。
8. エラーが発生したストーリーがあっても処理を継続し、最後にエラー一覧を報告する。
...
- フェーズ3: ストーリーフィードバック確認の件数（対象ストーリー数、検出フィードバック数、受け入れ条件更新数、追加タスク作成数、エラー）
- フェーズ4: 有効性チェックで除外された件数、依存先未完了で除外された件数、ストーリー間競合で除外された件数、競合回避で除外された件数、アサイン成功の件数（成功/エラー）、受け入れ条件チェック結果（対象ストーリー数、充足済み/未充足、追加タスク作成数）
