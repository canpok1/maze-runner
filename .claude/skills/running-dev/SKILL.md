...
4. `/coding` コマンドの手順を実行する（実装・テスト）。
    - 手順1で取得したIssue番号を引数として渡す。
5. `/reviewing` コマンドの手順を実行する（自己レビュー）。
6. `/creating-pr` コマンドの手順を実行する（PR作成）。
7. PRがマージされるまで、CIの結果やレビューコメントに応じて `/fixing-pr` コマンドを繰り返し実行する。
    - PRがマージされずにクローズされた場合は、その時点でユーザーに報告し、作業を中断する。
8. PRがマージされたら、`/running-retro` コマンドの手順を実行する（振り返り）。
